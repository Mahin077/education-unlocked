...
if (isset($_POST['edit'])) {
    ...
    if ($resutlChk->num_rows > 0) {
        if ($npassword == 'd41d8cd98f00b204e9800998ecf8427e') {
            $sql = "update user set username='$username',email='$email',password='$cpassword' where userid='$id'";
            $result = mysqli_query($conn, $sql);
            if ($result) {
            ...
            $sql = "update user set username='$username',email='$email',password='$npassword' where userid='$id'";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                echo "<script>alert('Updated');location.href='profile.php';</script";
            }
